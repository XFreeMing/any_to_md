...
def _split_document_into_fragments(
    ...
    for i, entry in enumerate(entries):
        ...
        start_idx = html_with_tokens.find(start_token)
        if start_idx == -1:
        ...
        fragments[entry.index] = html_with_tokens[start_idx:end_idx]
    return fragments
...
def _convert_by_spine(
    ...
    *,
    ...
    for index, item in enumerate(_iter_spine_documents(book), start=1):
        ...
        if include_metadata:
            parts.append(
                _metadata_block(book, item.get_id() or f"Chapter {index}", index)
            )
        ...
        chapter = Chapter(
            index=index,
            title=item.get_id() or f"Chapter {index}",
